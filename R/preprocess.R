...
preprocess_data <- function(dt) {
    dplyr::tbl_dt(dt) %>% dplyr::rename(
        species=organism, tissue_cell_line=cell_type,
...
#' 
#' @param dt tbl_dt as returned from preprocess_data
#' @return tbl_dt with columns: id, geo_id, gene, pert_type, species, tissue_cell_line
extract_description <- function(dt) {
    dt %>% dplyr::select(id, geo_id, gene, pert_type, species, tissue_cell_line)
}
